...
Investment.add({
	...
	currency: { type: Types.Select, options: 'BTC, BCH, ETH, LTC', initial: true },
	address: { type: String, initial: true },
	amount: { type: Types.Number, initial: true },
	type: { type: Types.Select, options: 'Investment, Referral Bonus, Bonus, Bounty', initial: true },
	usdValue: { type: Types.Money, initial: true },
	totalValue: { type: Types.Money, initial: true },
	tokenPrice: { type: Types.Money },
	tokenPurchased: { type: Types.Number },
			...
			client.getExchangeRates({
				...
				Investment.model.findOne({
					...
					if (findError) {
						...
						eden.list('Token').model.findOne({ remaining: {$gt:0} }).sort({remaining: 1}).exec(function(err, availableToken) {
							if(err){
								console.log(err);
							}else{
								if (investment.totalValue / availableToken.price > availableToken.remaining) {
								} else {
