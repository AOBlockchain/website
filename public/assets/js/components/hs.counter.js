...
;(function($){
	...
	$.HSCore.components.HSCounter = {
			...
			appear({
				...
				appear: function(el) {
					...
					var $item = $(el),
							...
							iterationValue = parseFloat(endValue / ((self.config['time'] / self.config['fps']))),
		...
		getCommaReducedValue: function(value, additionalText) {
			...
			return parseFloat(value / 1000) + additionalText;
		...
		getCommaSeparatedValue: function(value) {
			...
			if (this.parts.length > 0) {
				this.int = this.parts[0].toString();
				console.log(this.int);
				this.dec = this.parts[1].toString();
				console.log(this.dec);
			}

			switch(this.int.length || value.length) {
